...
EXPECTED_HEALTHCARE = {
    ...
    "Phase I residual mean": "-0.004",
    "Phase I residual standard deviation": "1.62",
    "Phase I residual ACF lag 1": "0.044",
    "Phase I residual ACF lag 24": "-0.041",
    "EWMA lambda": "0.2",
    "EWMA L": "3.3",
...
def verify_reported_diagnostics(output_dir: Path, mismatches: list[str]) -> None:
    path = output_dir / "baseline_diagnostics.json"
    if not path.exists():
        mismatches.append("missing baseline_diagnostics.json")
        return
    import json
    ...
    reported = data.get("manuscript_reported", {})
    for metric, value in expected.items():
        actual = reported.get(metric)
        if actual != value:
            mismatches.append(f"reported diagnostic {metric}: expected={value!r}, actual={actual!r}")
...
def main() -> None:
    ...
    verify_reported_diagnostics(output_dir, mismatches)
